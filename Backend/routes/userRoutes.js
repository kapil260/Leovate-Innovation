// ============================================================
// routes/userRoutes.js  — Recall AI v3.1 (Resend email)
// ============================================================
            ...
            <tr>
              <td style="padding:36px 40px;">
                ...
                <p style="margin:0 0 28px;font-size:14px;color:#6b7db3;text-align:center;">
                  Hi ${username || "there"}, use the code below to reset your Recall AI password.
                </p>
                <div style="background:rgba(159,167,255,0.07);border:2px dashed rgba(159,167,255,0.3);
                  ...
                  <p style="margin:0 0 8px;font-size:11px;text-transform:uppercase;
                    letter-spacing:0.15em;color:#6b7db3;">Your reset code</p>
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`[Recall AI] ✅ Password reset OTP sent to ${user.email}`);
    } catch (emailErr) {
...
router.post("/reset-password", async (req, res) => {
  try {
    ...
    await supabase.from("profiles").update({ password: hashedPassword }).eq("id", resetRecord.user_id);
...
// ── RESEND EMAIL VERIFICATION (legacy link-based) ─────────────
...
router.post("/verify-email", async (req, res) => {
  try {
    ...
    await supabase.from("profiles").update({ email_verified: true }).eq("id", verifyRecord.user_id);
    await supabase.from("email_verifications").update({ used: true }).eq("id", verifyRecord.id);
